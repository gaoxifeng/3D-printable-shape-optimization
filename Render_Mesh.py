...
class Render_Mesh():
    def __init__(self, mesh_dir, out_dir, mtl_override=None):
        ...
        print("Base mesh has %d triangles and %d vertices." % (self.ref_mesh.t_pos_idx.shape[0], self.ref_mesh.v_pos.shape[0]))
        ...
        os.makedirs(os.path.join(self.out_dir, "mesh"), exist_ok=True)
        os.makedirs(os.path.join(self.out_dir, "images"), exist_ok=True)

    def render(self, mvp, campos, resolution, mesh_scale=2.0):
        #lightpos = util.cosine_sample(campos) * RADIUS #This is not used when we generate the images
        params = {'mvp': mvp, 'lightpos': campos, 'campos': campos, 'resolution': [resolution, resolution], 'time': 0}
        _opt_ref = mesh.center_by_reference(self.render_ref_mesh.eval(params), self.ref_mesh_aabb, mesh_scale)
        with torch.no_grad():
            light_power = 0
            color_ref = render.render_mesh(self.glctx, _opt_ref, mvp, campos, campos, light_power, resolution,background=None)
...
def main(Batch_size, mesh_dir, out_dir, resolution):
    ...
    Render.render(mvp,campos, resolution)



if __name__ == "__main__":
    main(10,'data/f16/f16.obj','F16', 512)
