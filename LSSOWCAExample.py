...
def Dumbbell_Example_3D(res=(100, 100, 100), r=2, l=1):
    nelx, nely, nelz = res
    phiTensor = -torch.ones(res).cuda()
    phiFixedTensor = torch.ones((nelx + 1, nely + 1, nelz + 1)).cuda()
    phi = torch.ones([nelx+1, nely+1, nelz+1]).cuda()
...
if __name__ == "__main__":
    ...
    _, params = Dumbbell_Example_3D(r=0, l=3)
    # _, params = combine_two_mesh()
    sol = LevelSetShapeOptWorstCaseApproximation(h=1, dt=0.1, tau=1e-6,outputDetail=False, maxloop=100)
    ...
    #
    vertices, triangles = mcubes.marching_cubes(rho, 0.01)
    mesh = trimesh.Trimesh(vertices, triangles)
    _ = mesh.export('test.obj')
    # mesh.show()
